...
import { getPersonById } from "@/lib/actions/persons"
...
export default async function PersonProfilePage({ params }: PersonPageProps) {
    ...
    const person = result.data as any

    const totalOrders = person.orders?.length ?? 0
    const totalAmount = (person.orders ?? []).reduce((sum: number, o: any) => sum + (parseFloat(o.totalAmount) || 0), 0)
    const primaryPhone = person.contacts?.find((c: any) => c.type === "phone" && c.isPrimary) ?? person.contacts?.[0]

    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            {/* Breadcrumb */}
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Link href="/persons" className="hover:text-foreground transition-colors">الأشخاص</Link>
                <ArrowRight className="h-3 w-3" />
                <span className="text-foreground font-medium">{person.name ?? "بدون اسم"}</span>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-border/50 pb-6">
                ...
                <PersonSheet person={person} trigger={
            ...
            <div className="grid gap-4 md:grid-cols-3">
                {[
                    ...
                    { label: "تاريخ الانضمام", value: new Date(person.createdAt).toLocaleDateString("ar-SA", { year: "numeric", month: "long" }), icon: Calendar, color: "text-violet-600", bg: "bg-violet-500/10" },
                ].map((stat, i) => (
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                ...
                <div className="rounded-2xl border bg-card p-5 shadow-sm space-y-4">
                    ...
                    {person.contacts?.length > 0 ? person.contacts.map((c: any) => {
                ...
                <div className="rounded-2xl border bg-card p-5 shadow-sm space-y-4">
                    ...
                    {person.priceLabels?.length > 0 && (
                        <div className="flex gap-2 flex-wrap">
                            ...
                            {person.priceLabels.map((pl: any) => (
                                <Badge key={pl.priceLabelId} className="bg-muted text-muted-foreground border-0 text-xs">{pl.priceLabel.name}</Badge>
                            ))}
                        </div>
                    )}
                </div>
                ...
                {/* Group info */}
                {(person.groupName || person.groupNumber) && (
                    <div className="rounded-2xl border bg-card p-5 shadow-sm space-y-4">
                        <h2 className="font-semibold flex items-center gap-2 text-sm">
                            <Users className="h-4 w-4 text-primary" /> معلومات المجموعة
                        </h2>
                        {person.groupName && <p className="text-sm">{person.groupName}</p>}
                        {person.groupNumber && <p className="text-sm font-mono text-muted-foreground" dir="ltr">{person.groupNumber}</p>}
                    </div>
                )}
            </div>
