...
import { addTagToItem, removeTagFromItem } from "@/lib/actions/items"
...
export function QuickAddTag({
    ...
    const handleRemoveTag = async (e: React.MouseEvent, tagToRemove: string) => {
        ...
        try {
            const result = await removeTagFromItem(itemId, tagToRemove)
            if (result.success) {
                toast.success('تم إزالة الوسم', { description: `تم إزالة "#${tagToRemove}" من الصنف` })
                router.refresh()
            } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const result = await addTagToItem(itemId, trimmedTag)

            if (result.success) {
                toast.success('تم إضافة الوسم', { description: `تم إضافة "#${trimmedTag}" للصنف` })
                setNewTag("")
                setOpen(false)
                router.refresh()
            } else {
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        أضف وسم جديد لـ <span className="font-semibold text-foreground">{itemName}</span>
                    </DialogDescription>
                </DialogHeader>
