...
export function ServerPagination({
    ...
    return (
        <div className={cn(
            "flex items-center justify-between gap-4 flex-wrap px-2 py-3",
            "rounded-xl border border-border/40 bg-muted/10 backdrop-blur-sm",
            className
        )}>
            {/* ── Left: count + limit selector ── */}
            <div className="flex items-center gap-3">
                <span className="text-[12px] text-muted-foreground whitespace-nowrap select-none">
                    عرض{" "}
                ...
                {onLimitChange && (
                    ...
                    >
                        <SelectTrigger className="h-8 w-[100px] text-xs rounded-lg border-border/50 bg-background">
                            <SelectValue />
                        </SelectTrigger>
            ...
            {/* ── Center: page info (always visible) ── */}
            <div className="text-[11px] text-muted-foreground font-medium select-none whitespace-nowrap">
                صفحة{" "}
                <span className="font-bold text-foreground tabular-nums">{page}</span>
                {" "}من{" "}
                <span className="font-bold text-foreground tabular-nums">{Math.max(1, pages)}</span>
            </div>
