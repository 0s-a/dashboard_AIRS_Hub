...
import { Tag, Edit2, Trash2, X, Copy, LayoutGrid, Calculator, Plus } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Badge } from "@/components/ui/badge"
import { toast } from "sonner"
import { updateItemPrice, deleteItemPrice } from "@/lib/actions/items"
...
export function PriceListPanel({
    itemId, prices, itemUnits, priceLabels, currencies, onPricesChange
}: PriceListPanelProps) {
    ...
    const handleUpdate = () => {
        ...
        startTransition(async () => {
            const res = await updateItemPrice(editing.priceId, {
                value: val,
                priceLabelId: editing.priceLabelId,
                unitId: editing.unitId,
            })
            if (res.success && res.data) {
                onPricesChange((res.data as any).itemPrices || [])
                setEditing(null)
    ...
    const handleDelete = (priceId: string) => {
        startTransition(async () => {
            const res = await deleteItemPrice(priceId)
            if (res.success && res.data) {
                onPricesChange((res.data as any).itemPrices || [])
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-5 border-b border-border/40 bg-linear-to-r from-muted/30 to-transparent">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        <span className="text-base font-bold bg-linear-to-l from-foreground to-foreground/70 bg-clip-text text-transparent">تسعيرات الصنف</span>
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] text-muted-foreground">
                                الأسعار بالعملة الافتراضية{currencySymbol ? ` (${currencySymbol})` : ''} — التحويل عند الحاجة
                            </span>
                ...
                {!isAddingAuto && !isAddingSingle && (
                    <div className="flex gap-2">
                        ...
                        <Button size="sm" variant="outline"
                            ...
                            disabled={isPending || itemUnits.length === 0}>
            ...
            <div className="divide-y divide-border/30">
                {prices.length === 0 && !isAddingAuto && !isAddingSingle && (
                    <div className="flex flex-col items-center gap-4 py-20 text-center px-6">
                        ...
                        <div className="max-w-xs space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground leading-relaxed">ابدأ بإضافة تسعيرات مخصصة أو استخدم التسعير الذكي لتوليد الأسعار لجميع الوحدات آلياً.</p>
                        </div>
                        <Button size="sm" variant="default" className="gap-2 mt-2 rounded-xl"
                            onClick={() => setIsAddingAuto(true)} disabled={itemUnits.length === 0}>
                            ...
                            <table className="w-full text-right">
                                <thead>
                                    <tr className="border-b border-border/20 bg-background/30 backdrop-blur-xs">
                                        <th className="px-6 py-3 text-[10px] font-bold text-muted-foreground uppercase text-right w-1/3">الوحدة</th>
                                        <th className="px-6 py-3 text-[10px] font-bold text-muted-foreground uppercase text-center w-1/6">السعة</th>
                                        <th className="px-6 py-3 text-[10px] font-bold text-muted-foreground uppercase text-left w-1/3">السعر</th>
                                    ...
                                    {labelPrices.map((p) => (
                                        <tr key={p.id} className="group/row hover:bg-muted/10 transition-colors">
                                            {editing?.priceId === p.id ? (
                                                <td colSpan={4} className="px-6 py-4 bg-primary/[0.03]">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-1/3">
                                                            <Select value={editing.unitId} onValueChange={(val) => setEditing({ ...editing, unitId: val })}>
                                                                ...
                                                                <SelectContent>{itemUnits.map(pu => <SelectItem key={pu.id} value={pu.unitId}>{pu.unitName}</SelectItem>)}</SelectContent>
                                                            </Select>
                                                        </div>
                                                        <div className="w-1/3 relative">
                                                            <Input
                                                                ...
                                                                autoFocus
                                                            />
                                                            <div className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-muted-foreground/40">{currencySymbol}</div>
                                                        </div>
                                                ...
                                                <>
                                                    ...
                                                    <td className="px-6 py-4 text-center">
                                                        <span className="text-xs font-mono text-muted-foreground">×{itemUnits.find(u => u.unitId === p.unitId)?.conversionFactor || 1}</span>
                                                    </td>
                                                    <td className="px-6 py-4 text-left">
                                                        <div className="flex items-baseline justify-end gap-1.5">
                                                            <span className="text-xs font-bold text-muted-foreground/40 font-mono">{currencySymbol}</span>
                                                                ...
                                                                <Tooltip>
                                                                    <TooltipTrigger asChild>
                                                                        <Button size="icon" variant="ghost" className="h-7 w-7 rounded-lg text-muted-foreground hover:bg-primary/5 hover:text-primary"
                                                                            onClick={() => setEditing({ priceId: p.id, priceLabelId: p.priceLabelId, value: String(p.value), unitId: p.unitId })}>
                                                                ...
                                                                <AlertDialog>
                                                                    ...
                                                                    <AlertDialogContent className="rounded-3xl border-border/40 backdrop-blur-xl">
                                                                        <AlertDialogHeader>
                                                                            ...
                                                                            <AlertDialogDescription>
                                                                                أنت على وشك حذف سعر <span className="font-bold text-foreground">{p.unitName}</span> بقيمة <span className="font-bold text-foreground">{Number(p.value).toFixed(2)} {currencySymbol}</span> نهائياً.
                                                                            </AlertDialogDescription>
                                                                        </AlertDialogHeader>
                ...
                <div className="mt-12 mb-6 text-center space-y-4 px-6">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-muted/30 border border-border/40 text-[10px] font-medium text-muted-foreground/60 backdrop-blur-xs">
                        ...
                        تسعير بعملة واحدة مع تحويل عند الحاجة
                    </div>
                </div>
            </div>
            ...
            {copyDialog && (
                <CopyPriceDialog
                    ...
                    onConfirm={async (toLabelId, pct) => {
                        const { copyPriceLabelPrices } = await import('@/lib/actions/items')
                        startTransition(async () => {
                            const res = await copyPriceLabelPrices(itemId, copyDialog.fromLabelId, toLabelId, pct)
                            if (res.success && res.data) {
                                onPricesChange((res.data as any).itemPrices || [])
