...
import { addItemPricesForAllUnits } from "@/lib/actions/items"
...
export function SmartPricingWizard({
    itemId, itemUnits, priceLabels, currencies, onComplete, onCancel
    ...
    const handleSubmit = () => {
        ...
        if (itemUnits.length === 0) return toast.error("أضف وحدات للصنف أولاً")

        startTransition(async () => {
            const res = await addItemPricesForAllUnits(itemId, {
                ...
                <div className="md:col-span-5 space-y-2">
                    <label className="text-[11px] font-bold text-muted-foreground/80 flex items-center justify-between">
                        ...
                        <Badge variant="outline" className="h-4 text-[9px] px-1 bg-muted/30">
                            {baseCurrency?.symbol ?? "—"} · {itemUnits.find(u => u.isBase)?.unitName || "لا توجد وحدة"}
                        </Badge>
                    </label>
