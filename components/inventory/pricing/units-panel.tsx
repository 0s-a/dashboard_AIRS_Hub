...
import { setProductUnits } from "@/lib/actions/inventory"
import type { ProductUnitEntry } from "@/lib/types/product"

// ─────────────────────────────────────────────────────────────
// Units Panel — Select and configure product units
// ─────────────────────────────────────────────────────────────
...
interface UnitsPanelProps {
    ...
    onUnitsChange: (units: ProductUnitEntry[]) => void
}

export function UnitsPanel({ productId, productUnits, sysUnits, onUnitsChange }: UnitsPanelProps) {
    ...
    const saveUnits = (unitsToSave = productUnits) => {
        startTransition(async () => {
            const res = await setProductUnits(productId, unitsToSave.map(u => ({
                unitId: u.unitId,
                isBase: u.isBase,
            ...
            if (res.success && res.data) {
                onUnitsChange((res.data as any).productUnits || [])
    ...
    const isDirty = JSON.stringify(productUnits) !== JSON.stringify(productUnits) // always false - save is explicit
