...
import { Trash2, Edit, Copy, ExternalLink, MoreHorizontal, UserCheck, UserX, Coins, AlertTriangle } from "lucide-react"
...
export type CustomerRow = Customer & {
    ...
    priceLabel?: { name?: string; customerType?: string | null } | null
    resolvedCurrencies?: { symbol?: string; code?: string; name?: string }[]
}
...
function getContacts(customer: CustomerRow): ContactRecord[] {
...
export const columns: ColumnDef<CustomerRow>[] = [
    ...
    // ──────────────────────────────────────
    {
        ...
        cell: ({ row }) => {
            ...
            const tags: string[] = (row.original.tags || []).map((pt) => pt.tag?.name ?? '')
    ...
    {
        id: "currencies",
        accessorFn: (row) => row.resolvedCurrencies,
        header: "العملات",
        enableColumnFilter: true,
        meta: {
        ...
        filterFn: (row, _columnId, filterValue: string) => {
            ...
            return resolvedCurrencies.some((c) => c.symbol === filterValue || c.code === filterValue || c.name?.includes(filterValue))
        },
        size: 170,
        minSize: 130,
        maxSize: 210,
        cell: ({ row }) => {
            ...
            return (
                <div className="flex flex-wrap gap-1.5 items-center justify-center max-w-[150px] text-xs text-amber-700 dark:text-amber-400 font-medium">
                    ...
                    <span className="truncate" title={displayCurrencies.map((c) => c.symbol).join("، ")}>
                        {displayCurrencies.map((c) => c.symbol).join("، ")}
                    </span>
                    {remaining > 0 && (
    ...
    // ──────────────────────────────────────
    {
        ...
        meta: {
            ...
            booleanLabels: { true: "نشط", false: "غير نشط", all: "الكل" },
            align: 'center' as const,
        },
        filterFn: "boolean",
        size: 100,
